      ...
      <main className="bg-white min-h-screen">
        ...
        <div className="relative w-full bg-gray-200 overflow-hidden" style={{ height: 'calc(100dvh - 67px)' }}>
          {category?.featuredImage && (
            <img
              src={optimizeCloudinaryUrl(category.featuredImage, 1920)}
              srcSet={cloudinarySrcSet(category.featuredImage, 1920)}
              alt={category.name}
                  ...
                  <Link key={post.id} href={`/blog/${post.slug}`} className="bg-white overflow-hidden group">
                    {post.featuredImage && (
                      <div className="relative w-full aspect-square bg-gray-200 overflow-hidden">
                        <img
                          src={optimizeCloudinaryUrl(post.featuredImage, 800)}
                          srcSet={cloudinarySrcSet(post.featuredImage, 800)}
                          alt={post.title}
