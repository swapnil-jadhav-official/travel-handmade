...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [allPosts, testimonials, travellerData, settings] =
          await Promise.all([
            getAllPostsTyped(),
            getTestimonials(),
            getTravellers(),
  ...
  const travelLivingArticles = postsAsArticles
    .filter((p) => p.category === "travel-living")
    .slice(0, 3);
  const adventureArticles = postsAsArticles
    .filter((p) => p.category === "adventure-wildlife")
    .slice(0, 3);
  const foodDrinksArticles = postsAsArticles.filter(
    (p) => p.category === "food-drinks",
  );
  const retreatsArticles = postsAsArticles
    .filter((p) => p.category === "retreats")
    .slice(0, 3);
  const wellnessArticles = postsAsArticles
    .filter((p) => p.category === "wellness")
    .slice(0, 3);
  const changeMakerArticles = postsAsArticles
    .filter((p) => p.category === "changemaker")
    .slice(0, 3);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <main className="flex-1 w-full space-y-8 lg:space-y-12">
        ...
        {!loading && testimonialsList.length > 0 && (
          <ChangeMaker
            testimonials={testimonialsList}
            featuredVideo={featuredVideo}
          />
        )}
        {!loading && travellersList.length > 0 && (
          <TravellerSection travellers={travellersList} />
        )}
      </main>
      <Footer />
    </div>
